...
public class QueueReceiverMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            ReceiverThread thread = new ReceiverThread(session, requestMessage);
            thread.start();
        }
        // メッセージの受信を停止する
        // conn.stop();
...
class ReceiverThread extends Thread {
    ...
    public ReceiverThread(QueueSession session, TextMessage requestMessage) {
    ...
    public void run () {
        try {
            ...
            Integer waitTime = Integer.parseInt(text);
            while (0 < waitTime) {
                System.out.print(".");
                try {
                    Thread.sleep(1000);
